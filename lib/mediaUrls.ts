...
type CloudflareExtra = {
  cloudflareAccountHash?: string;
  cloudflareStreamSubdomain?: string;
};

const extra = (Constants.expoConfig?.extra ?? {}) as CloudflareExtra;

const ACCOUNT_HASH =
  process.env.EXPO_PUBLIC_CF_ACCOUNT_HASH ?? extra.cloudflareAccountHash ?? '';
const STREAM_SUBDOMAIN = (
  process.env.EXPO_PUBLIC_CF_STREAM_SUBDOMAIN ?? extra.cloudflareStreamSubdomain ?? ''
...
export function imageUrl(idOrUrl: string | null | undefined, variant: ImageVariant = 'public'): string | null {
